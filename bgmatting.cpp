...
// 在调用之前应该检查is_valid
cv::Mat BgMatting::forward(const cv::Mat img, const cv::Mat bgr)
{
    if(img.rows!=bgr.rows || img.cols!=bgr.cols)
    ...
    torch::Tensor bgrTensor=torch::from_blob(bgr.data,{1,bgr.rows,bgr.cols,3},torch::kU8);
    bgrTensor=bgrTensor.permute({0,3,1,2}).to(device).to(torch::kFloat16).div(255.0);
